...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('confirmEdit').addEventListener('click', async function() {
        ...
        try {
            // Reference to the Room_COE document in the Firestore
            const roomDocRef = doc(db, 'Room_Data', 'Room_COE', 'rooms', docId);
    ...
    async function doesRoomExist(roomNumber, campus, roomType, excludeDocId = null) {
        try {
            const roomDocRef = doc(db, 'Room_Data', 'Room_COE');
    ...
    async function saveRoomToFirestore(campus, roomType, roomNumber) {
        try {
            // Reference to the Room_COE document
            const roomDocRef = doc(db, 'Room_Data', 'Room_COE');
    
            // Reference to the sub-collection 'rooms' under Room_COE document
            const roomCollectionRef = collection(roomDocRef, 'rooms');
            ...
            if (!lastDoc.empty) {
                ...
                const lastId = parseInt(lastDocData.id.replace('roomCOE_', '')); // Extract the numeric part of the ID
                newId = lastId + 1; // Increment to get the new ID
            }
    
            // New document ID with leading zeros (e.g., sectionCOE_001)
            const newDocId = `roomCOE_${newId.toString().padStart(3, '0')}`;
