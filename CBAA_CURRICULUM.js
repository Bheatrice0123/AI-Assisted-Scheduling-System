...
async function uploadCSVDataToFirestore(csvData) {
    const docRef = doc(db, "tempCurriculum", "tempCurriculum_CBAA");
...
// Function to fetch and display data from the tempCurriculum_CBAA document
...
async function checkIfCurriculumExists(program, batch) {
    const curriculumRef = collection(db, "mainCurriculum", "mainCurriculum_CBAA", program);
...
async function confirmAndSaveCurriculum() {
    ...
    if (confirm("Are you sure you want to save this curriculum?")) {
        ...
        try {
            const programCollectionRef = collection(db, "mainCurriculum", "mainCurriculum_CBAA", program);
            ...
            // Clear fields in tempCurriculum_CBAA (temporary storage)
            const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_CBAA");
...
async function getNextDocumentId() {
    ...
    snapshot.forEach(doc => {
        const id = doc.id.replace("mainCurriculum_CBAA", ""); // Remove the prefix to get the number
    ...
    const nextId = `mainCurriculum_CBAA${maxId + 1}`;
...
// Function to delete data from tempCurriculum_CBAA and clear the table
async function deleteCurriculum() {
    ...
    // Clear data from Firestore (tempCurriculum_CBAA)
    const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_CBAA");
...
document.getElementById('viewCurriculumBtn').addEventListener('click', function() {
    window.location.href = 'CBAA_VIEW_CURRICULUM.html';
});
