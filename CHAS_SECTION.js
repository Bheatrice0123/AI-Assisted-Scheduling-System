...
document.getElementById('courseSectionForm').addEventListener('submit', async function(event) {
    ...
    try {
        // Reference to the Sections sub-collection inside Section_CHAS under Section_Data
        const sectionCHASCollection = collection(db, 'Section_Data', 'Section_CHAS', 'Sections');
        console.log("Section_CHAS -> Sections sub-collection reference: ", sectionCHASCollection); // Debugging output
        ...
        const sectionPromises = sectionNames.map(async (sectionName, index) => {
            ...
            const sectionDocRef = doc(db, 'Section_Data', 'Section_CHAS', 'Sections', sectionName);
...
async function checkExistingSections(program, yearLevelSelected) {
    try {
        const sectionsCollection = collection(db, 'Section_Data', 'Section_CHAS', 'Sections');
...
async function deleteRow(button, sectionName) {
    ...
    try {
        ...
        const querySnapshot = await getDocs(collection(db, 'Section_Data', 'Section_CHAS', 'Sections'));
