...
async function fetchFaculty() {
    const coeFacultyData = [];
    const facultyCollection = collection(db, 'Faculty_Data', 'COE', 'COE_Faculty');
    const querySnapshot = await getDocs(facultyCollection);

    querySnapshot.forEach((docSnap) => {
        const data = docSnap.data();
        coeFacultyData.push(data);
    });

    return coeFacultyData;
}
...
async function fetchSections() {
    const sectionData = [];
    const sectionCollection = collection(db, 'Section_Data', 'Section_COE', 'Sections');
...
async function fetchCourses(selectedSemester, academicYear) {
    ...
    const curriculumPrograms = ['BSIE', 'BSECE', 'BSCPE']; // Add other programs if needed

    // Loop through each program in mainCurriculum_COE
    for (const program of curriculumPrograms) {
        const programCollection = collection(db, 'mainCurriculum', 'mainCurriculum_COE', program);
...
function initializePopulation(coeFacultyData, coedFacultyData, courseData, sectionData, populationSize) {
    const population = [];
    ...
    for (let i = 0; i < populationSize; i++) {
        ...
        [...coeFacultyData, ...coedFacultyData].forEach(faculty => {
        ...
        sectionData.forEach(section => {
            courseData.forEach(course => {
                ...
                if (course.GenEd === "TRUE") {
                    ...
                    experiencedFaculty = coeFacultyData.filter(faculty =>
                        faculty.isActive &&
                        faculty.past_courses &&
                        faculty.past_courses.some(c => c.course_code === course["Course Code"]) &&
                        (facultyHours[faculty.name] + parseFloat(course["Set Hours"]) <= faculty.max_hours)
                ...
                if (!assigned) {
                    ...
                    let eligibleFaculty = (course.GenEd === "TRUE" ? coedFacultyData : coeFacultyData).filter(faculty =>
                        faculty.isActive &&
    ...
    for (let i = 0; i < chromosome.length; i++) {
        if (Math.random() < mutationRate) {
            ...
            if (course.genEd === "TRUE") {
                ...
                eligibleFaculty = coeFacultyData.filter(faculty =>
                    faculty.isActive &&
                    (faculty.max_hours >= facultyHours[faculty.name] + course.set_hours)
                );
            }
...
export function geneticAlgorithm(coeFacultyData, coedFacultyData, courseData, sectionData, populationSize = 50, generations = 100, mutationRate = 0.05) {
    ...
    let population = initializePopulation(coeFacultyData, coedFacultyData, clonedCourseData, sectionData, populationSize);
    ...
    for (let gen = 0; gen < generations; gen++) {
        ...
        const fitnessScores = population.map((chromosome, index) => {
            ...
            return fitnessFunction(chromosome, [...coeFacultyData, ...coedFacultyData], clonedCourseData);
        });
        ...
        while (newPopulation.length < populationSize) {
            ...
            const mutatedChild1 = mutation(child1, [...coeFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
            const mutatedChild2 = mutation(child2, [...coeFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
    ...
    const finalFitnessScores = population.map(chromosome => fitnessFunction(chromosome, [...coeFacultyData, ...coedFacultyData], clonedCourseData));
...
async function saveBestAssignment(bestChromosomeWithRooms, fitness, academicYear, selectedSemester) {
    ...
    const assignmentDocRef = doc(db, 'GenerateSchedule', 'COE_Schedule');
...
async function fetchRooms() {
    const roomData = [];
    const roomCollection = collection(db, 'Room_Data', 'Room_COE', 'rooms');
...
async function fetchAllDepartmentSchedules(academicYear, semester) {
    const departments = ['CBAA', 'COED', 'COE', 'CHAS', 'COE'];  // Add any other departments
    const schedules = [];
...
export async function saveAndClearSchedule() {
    const sourceDocRef = doc(db, "GenerateSchedule", "COE_Schedule");
    ...
    try {
        ...
        if (scheduleDoc.exists()) {
            ...
            if (!academicYear || !selectedSemester) {
                console.error("Missing academicYear or selectedSemester in COE_Schedule document.");
                return;
            }
            ...
            console.log(`Schedule copied to finalSchedule > A.Y. ${academicYear} > ${selectedSemester} > COE > schedule.`);
            ...
            console.log("Fields cleared in GenerateSchedule > COE_Schedule.");
        } else {
...
export async function main() {
    try {
        ...
        console.log('Fetching COE faculty data...');
        const coeFacultyData = await fetchFaculty();
        console.log(`Fetched ${coeFacultyData.length} COE faculty members.`);
        ...
        let { bestChromosome, bestFitness } = geneticAlgorithm(coeFacultyData, coedFacultyData, courseData, sectionData, 50, 100, 0.05);
