...
async function uploadCSVDataToFirestore(csvData) {
    const docRef = doc(db, "tempCurriculum", "tempCurriculum_CHAS");
...
// Function to fetch and display data from the tempCurriculum_CHAS document
...
async function checkIfCurriculumExists(program, batch) {
    const curriculumRef = collection(db, "mainCurriculum", "mainCurriculum_CHAS", program);
...
async function confirmAndSaveCurriculum() {
    ...
    if (confirm("Are you sure you want to save this curriculum?")) {
        ...
        try {
            const programCollectionRef = collection(db, "mainCurriculum", "mainCurriculum_CHAS", program);
            ...
            // Clear fields in tempCurriculum_CHAS (temporary storage)
            const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_CHAS");
...
async function getNextDocumentId() {
    ...
    snapshot.forEach(doc => {
        const id = doc.id.replace("mainCurriculum_CHAS", ""); // Remove the prefix to get the number
    ...
    const nextId = `mainCurriculum_CHAS${maxId + 1}`;
...
// Function to delete data from tempCurriculum_CHAS and clear the table
async function deleteCurriculum() {
    ...
    // Clear data from Firestore (tempCurriculum_CHAS)
    const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_CHAS");
...
document.getElementById('viewCurriculumBtn').addEventListener('click', function() {
    window.location.href = 'CHAS_VIEW_CURRICULUM.html';
});
