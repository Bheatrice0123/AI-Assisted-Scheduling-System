...
async function uploadCSVDataToFirestore(csvData) {
    const docRef = doc(db, "tempCurriculum", "tempCurriculum_CAS");
...
async function checkIfCurriculumExists(program, batch) {
    const curriculumRef = collection(db, "mainCurriculum", "mainCurriculum_CAS", program);
...
async function confirmAndSaveCurriculum() {
   ...
   // Check if any of the Set Hours is 0
    ...
    if (confirm("Are you sure you want to save this curriculum?")) {
        ...
        try {
            const programCollectionRef = collection(db, "mainCurriculum", "mainCurriculum_CAS", program);
            ...
            // Clear fields in tempCurriculum_CAS (temporary storage)
            const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_CAS");
...
async function getNextDocumentId() {
    ...
    snapshot.forEach(doc => {
        const id = doc.id.replace("mainCurriculum_CAS", ""); // Remove the prefix to get the number
    ...
    const nextId = `mainCurriculum_CAS${maxId + 1}`;
...
document.getElementById('viewCurriculumBtn').addEventListener('click', function() {
    window.location.href = 'CAS_VIEW_CURRICULUM.html';
});
