...
document.addEventListener('DOMContentLoaded', function () {
    ...
    facultyForm.addEventListener('submit', function (e) {
        ...
        const facultyData = {
            ...
            department: 'CAS', // Automatically set department as "CCS"
        ...
        if (editingDocId) {
            ...
            db.collection('Faculty_Data')
              .doc('CAS')
              .collection('CAS_Faculty')
              .get()
              .then((snapshot) => {
                  const count = snapshot.size; // Get the number of existing documents
                  const nextId = `CASFaculty_${String(count + 1).padStart(3, '0')}`; // Generate next ID (CCSFaculty_001, etc.)
    ...
    /**
     * Sets up a real-time listener to the CCS_Faculty collection.
