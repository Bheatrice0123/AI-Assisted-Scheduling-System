...
async function fetchFaculty() {
    const cbaaFacultyData = [];
    const facultyCollection = collection(db, 'Faculty_Data', 'CBAA', 'CBAA_Faculty');
    const querySnapshot = await getDocs(facultyCollection);

    querySnapshot.forEach((docSnap) => {
        const data = docSnap.data();
        cbaaFacultyData.push(data);
    });

    return cbaaFacultyData;
}
...
async function fetchSections() {
    const sectionData = [];
    const sectionCollection = collection(db, 'Section_Data', 'Section_CBAA', 'Sections');
...
async function fetchCourses(selectedSemester, academicYear) {
    ...
    const curriculumPrograms = ['BSA', 'BSBA-F', 'BSBA-M']; // Add other programs if needed

    // Loop through each program in mainCurriculum_CBAA
    for (const program of curriculumPrograms) { 
        const programCollection = collection(db, 'mainCurriculum', 'mainCurriculum_CBAA', program);
...
function initializePopulation(cbaaFacultyData, coedFacultyData, courseData, sectionData, populationSize) {
    const population = [];
    ...
    for (let i = 0; i < populationSize; i++) {
        ...
        [...cbaaFacultyData, ...coedFacultyData].forEach(faculty => {
        ...
        sectionData.forEach(section => {
            courseData.forEach(course => {
                ...
                if (course.GenEd === "TRUE") {
                    ...
                    experiencedFaculty = cbaaFacultyData.filter(faculty =>
                        faculty.isActive &&
                        faculty.past_courses &&
                        faculty.past_courses.some(c => c.course_code === course["Course Code"]) &&
                        (facultyHours[faculty.name] + parseFloat(course["Set Hours"]) <= faculty.max_hours)
                ...
                if (!assigned) {
                    ...
                    let eligibleFaculty = (course.GenEd === "TRUE" ? coedFacultyData : cbaaFacultyData).filter(faculty =>
                        faculty.isActive &&
    ...
    for (let i = 0; i < chromosome.length; i++) {
        if (Math.random() < mutationRate) {
            ...
            if (course.genEd === "TRUE") {
                ...
                eligibleFaculty = cbaaFacultyData.filter(faculty =>
                    faculty.isActive &&
                    (faculty.max_hours >= facultyHours[faculty.name] + course.set_hours)
                );
            }
...
export function geneticAlgorithm(cbaaFacultyData, coedFacultyData, courseData, sectionData, populationSize = 50, generations = 100, mutationRate = 0.05) {
    ...
    let population = initializePopulation(cbaaFacultyData, coedFacultyData, clonedCourseData, sectionData, populationSize);
    ...
    for (let gen = 0; gen < generations; gen++) {
        ...
        const fitnessScores = population.map((chromosome, index) => {
            ...
            return fitnessFunction(chromosome, [...cbaaFacultyData, ...coedFacultyData], clonedCourseData);
        });
        ...
        while (newPopulation.length < populationSize) {
            ...
            const mutatedChild1 = mutation(child1, [...cbaaFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
            const mutatedChild2 = mutation(child2, [...cbaaFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
    ...
    const finalFitnessScores = population.map(chromosome => fitnessFunction(chromosome, [...cbaaFacultyData, ...coedFacultyData], clonedCourseData));
...
async function saveBestAssignment(bestChromosomeWithRooms, fitness, academicYear, selectedSemester) {
    ...
    const assignmentDocRef = doc(db, 'GenerateSchedule', 'CBAA_Schedule');
...
async function fetchRooms() {
    const roomData = [];
    const roomCollection = collection(db, 'Room_Data', 'Room_CBAA', 'rooms');
...
async function fetchAllDepartmentSchedules(academicYear, semester) {
    const departments = ['CAS', 'COED', 'COE', 'CHAS', 'CCS'];  // Add any other departments
    const schedules = [];
...
export async function saveAndClearSchedule() {
    const sourceDocRef = doc(db, "GenerateSchedule", "CBAA_Schedule");
    ...
    try {
        ...
        if (scheduleDoc.exists()) {
            ...
            if (!academicYear || !selectedSemester) {
                console.error("Missing academicYear or selectedSemester in CBAA_Schedule document.");
                return;
            }
            ...
            console.log(`Schedule copied to finalSchedule > A.Y. ${academicYear} > ${selectedSemester} > CBAA > schedule.`);
            ...
            console.log("Fields cleared in GenerateSchedule > CBAA_Schedule.");
        } else {
...
export async function main() {
    try {
        ...
        console.log('Fetching  CBAA faculty data...');
        const cbaaFacultyData = await fetchFaculty();
        console.log(`Fetched ${cbaaFacultyData.length} CBAA faculty members.`);
        ...
        let { bestChromosome, bestFitness } = geneticAlgorithm(cbaaFacultyData, coedFacultyData, courseData, sectionData, 50, 100, 0.05);
