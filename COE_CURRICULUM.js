...
async function uploadCSVDataToFirestore(csvData) {
    const docRef = doc(db, "tempCurriculum", "tempCurriculum_COE");
...
// Function to fetch and display data from the tempCurriculum_COE document
...
async function checkIfCurriculumExists(program, batch) {
    const curriculumRef = collection(db, "mainCurriculum", "mainCurriculum_COE", program);
...
async function confirmAndSaveCurriculum() {
    ...
    if (confirm("Are you sure you want to save this curriculum?")) {
        ...
        try {
            const programCollectionRef = collection(db, "mainCurriculum", "mainCurriculum_COE", program);
            ...
            // Clear fields in tempCurriculum_COE (temporary storage)
            const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_COE");
...
async function getNextDocumentId() {
    ...
    snapshot.forEach(doc => {
        const id = doc.id.replace("mainCurriculum_COE", ""); // Remove the prefix to get the number
    ...
    const nextId = `mainCurriculum_COE${maxId + 1}`;
...
// Function to delete data from tempCurriculum_COE and clear the table
async function deleteCurriculum() {
    ...
    // Clear data from Firestore (tempCurriculum_COE)
    const tempDocRef = doc(db, "tempCurriculum", "tempCurriculum_COE");
...
document.getElementById('viewCurriculumBtn').addEventListener('click', function() {
    window.location.href = 'COE_VIEW_CURRICULUM.html';
});
