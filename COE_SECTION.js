...
document.getElementById('courseSectionForm').addEventListener('submit', async function(event) {
    ...
    try {
        // Reference to the Sections sub-collection inside Section_COE under Section_Data
        const sectionCOECollection = collection(db, 'Section_Data', 'Section_COE', 'Sections');
        console.log("Section_COE -> Sections sub-collection reference: ", sectionCOECollection); // Debugging output
        ...
        const sectionPromises = sectionNames.map(async (sectionName, index) => {
            ...
            const sectionDocRef = doc(db, 'Section_Data', 'Section_COE', 'Sections', sectionName);
...
async function checkExistingSections(program, yearLevelSelected) {
    try {
        const sectionsCollection = collection(db, 'Section_Data', 'Section_COE', 'Sections');
...
async function deleteRow(button, sectionName) {
    ...
    try {
        ...
        const querySnapshot = await getDocs(collection(db, 'Section_Data', 'Section_COE', 'Sections'));
