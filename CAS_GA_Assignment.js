...
async function fetchFaculty() {
    const casFacultyData = [];
    const facultyCollection = collection(db, 'Faculty_Data', 'CAS', 'CAS_Faculty');
    const querySnapshot = await getDocs(facultyCollection);

    querySnapshot.forEach((docSnap) => {
        const data = docSnap.data();
        casFacultyData.push(data);
    });

    return casFacultyData;
}
...
async function fetchSections() {
    const sectionData = [];
    const sectionCollection = collection(db, 'Section_Data', 'Section_CAS', 'Sections');
...
async function fetchCourses(selectedSemester, academicYear) {
    ...
    const curriculumPrograms = ['BSPSY']; // Add other programs if needed

    // Loop through each program in mainCurriculum_CCS
    for (const program of curriculumPrograms) {
        const programCollection = collection(db, 'mainCurriculum', 'mainCurriculum_CAS', program);
...
function initializePopulation(casFacultyData, coedFacultyData, courseData, sectionData, populationSize) {
    const population = [];
    ...
    for (let i = 0; i < populationSize; i++) {
        ...
        [...casFacultyData, ...coedFacultyData].forEach(faculty => {
        ...
        sectionData.forEach(section => {
            courseData.forEach(course => {
                ...
                if (course.GenEd === "TRUE") {
                    ...
                    experiencedFaculty = casFacultyData.filter(faculty =>
                        faculty.isActive &&
                        faculty.past_courses &&
                        faculty.past_courses.some(c => c.course_code === course["Course Code"]) &&
                        (facultyHours[faculty.name] + parseFloat(course["Set Hours"]) <= faculty.max_hours)
                ...
                if (!assigned) {
                    ...
                    let eligibleFaculty = (course.GenEd === "TRUE" ? coedFacultyData : casFacultyData).filter(faculty =>
                        faculty.isActive &&
    ...
    for (let i = 0; i < chromosome.length; i++) {
        if (Math.random() < mutationRate) {
            ...
            if (course.genEd === "TRUE") {
                ...
                eligibleFaculty = casFacultyData.filter(faculty =>
                    faculty.isActive &&
                    (faculty.max_hours >= facultyHours[faculty.name] + course.set_hours)
                );
            }
...
export function geneticAlgorithm(casFacultyData, coedFacultyData, courseData, sectionData, populationSize = 50, generations = 100, mutationRate = 0.05) {
    ...
    let population = initializePopulation(casFacultyData, coedFacultyData, clonedCourseData, sectionData, populationSize);
    ...
    for (let gen = 0; gen < generations; gen++) {
        ...
        const fitnessScores = population.map((chromosome, index) => {
            ...
            return fitnessFunction(chromosome, [...casFacultyData, ...coedFacultyData], clonedCourseData);
        });
        ...
        while (newPopulation.length < populationSize) {
            ...
            const mutatedChild1 = mutation(child1, [...casFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
            const mutatedChild2 = mutation(child2, [...casFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
    ...
    const finalFitnessScores = population.map(chromosome => fitnessFunction(chromosome, [...casFacultyData, ...coedFacultyData], clonedCourseData));
...
async function saveBestAssignment(bestChromosomeWithRooms, fitness, academicYear, selectedSemester) {
    ...
    const assignmentDocRef = doc(db, 'GenerateSchedule', 'CAS_Schedule');
...
async function fetchRooms() {
    const roomData = [];
    const roomCollection = collection(db, 'Room_Data', 'Room_CAS', 'rooms');
...
export async function saveAndClearSchedule() {
    const sourceDocRef = doc(db, "GenerateSchedule", "CAS_Schedule");
    ...
    try {
        ...
        if (scheduleDoc.exists()) {
            ...
            if (!academicYear || !selectedSemester) {
                console.error("Missing academicYear or selectedSemester in CAS_Schedule document.");
                return;
            }
            ...
            console.log(`Schedule copied to finalSchedule > A.Y. ${academicYear} > ${selectedSemester} > CAS > schedule.`);
            ...
            console.log("Fields cleared in GenerateSchedule > CAS_Schedule.");
        } else {
...
export async function main() {
    try {
        ...
        console.log('Fetching CAS faculty data...');
        const casFacultyData = await fetchFaculty();
        console.log(`Fetched ${casFacultyData.length} CAS faculty members.`);
        ...
        let { bestChromosome, bestFitness } = geneticAlgorithm(casFacultyData, coedFacultyData, courseData, sectionData, 50, 100, 0.05);
