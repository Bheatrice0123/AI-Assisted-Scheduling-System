...
document.getElementById('courseSectionForm').addEventListener('submit', async function(event) {
    ...
    try {
        // Reference to the Sections sub-collection inside Section_COED under Section_Data
        const sectionCOEDCollection = collection(db, 'Section_Data', 'Section_COED', 'Sections');
        console.log("Section_COED -> Sections sub-collection reference: ", sectionCOEDCollection); // Debugging output
        ...
        const sectionPromises = sectionNames.map(async (sectionName, index) => {
            ...
            const sectionDocRef = doc(db, 'Section_Data', 'Section_COED', 'Sections', sectionName);
...
async function checkExistingSections(program, yearLevelSelected) {
    try {
        const sectionsCollection = collection(db, 'Section_Data', 'Section_COED', 'Sections');
...
async function deleteRow(button, sectionName) {
    ...
    try {
        ...
        const querySnapshot = await getDocs(collection(db, 'Section_Data', 'Section_COED', 'Sections'));
