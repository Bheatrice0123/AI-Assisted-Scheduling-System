...
async function fetchFaculty() {
    const chasFacultyData = [];
    const facultyCollection = collection(db, 'Faculty_Data', 'CHAS', 'CHAS_Faculty');
    const querySnapshot = await getDocs(facultyCollection);

    querySnapshot.forEach((docSnap) => {
        const data = docSnap.data();
        chasFacultyData.push(data);
    });

    return chasFacultyData;
}
...
async function fetchSections() {
    const sectionData = [];
    const sectionCollection = collection(db, 'Section_Data', 'Section_CHAS', 'Sections');
...
async function fetchCourses(selectedSemester, academicYear) {
    ...
    const curriculumPrograms = ['BSN']; // Add other programs if needed

    // Loop through each program in mainCurriculum_CHAS
    for (const program of curriculumPrograms) {
        const programCollection = collection(db, 'mainCurriculum', 'mainCurriculum_CHAS', program);
        ...
        querySnapshot.forEach((docSnap) => {
            ...
            csvRows.forEach((course) => {
                ...
                if (course["Course Code"] && course["Course Code"].includes("RLE")) {
...
function initializePopulation(chasFacultyData, coedFacultyData, courseData, sectionData, populationSize) {
    const population = [];
    ...
    for (let i = 0; i < populationSize; i++) {
        ...
        [...chasFacultyData, ...coedFacultyData].forEach(faculty => {
        ...
        sectionData.forEach(section => {
            courseData.forEach(course => {
                ...
                if (course.GenEd === "TRUE") {
                    ...
                    experiencedFaculty = chasFacultyData.filter(faculty =>
                        faculty.isActive &&
                        faculty.past_courses &&
                        faculty.past_courses.some(c => c.course_code === course["Course Code"]) &&
                        (facultyHours[faculty.name] + parseFloat(course["Set Hours"]) <= faculty.max_hours)
                ...
                if (!assigned) {
                    ...
                    let eligibleFaculty = (course.GenEd === "TRUE" ? coedFacultyData : chasFacultyData).filter(faculty =>
                        faculty.isActive &&
    ...
    for (let i = 0; i < chromosome.length; i++) {
        if (Math.random() < mutationRate) {
            ...
            if (course.genEd === "TRUE") {
                ...
                eligibleFaculty = chasFacultyData.filter(faculty =>
                    faculty.isActive &&
                    (faculty.max_hours >= facultyHours[faculty.name] + course.set_hours)
                );
            }
...
export function geneticAlgorithm(chasFacultyData, coedFacultyData, courseData, sectionData, populationSize = 50, generations = 100, mutationRate = 0.05) {
    ...
    let population = initializePopulation(chasFacultyData, coedFacultyData, clonedCourseData, sectionData, populationSize);
    ...
    for (let gen = 0; gen < generations; gen++) {
        ...
        const fitnessScores = population.map((chromosome, index) => {
            ...
            return fitnessFunction(chromosome, [...chasFacultyData, ...coedFacultyData], clonedCourseData);
        });
        ...
        while (newPopulation.length < populationSize) {
            ...
            const mutatedChild1 = mutation(child1, [...chasFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
            const mutatedChild2 = mutation(child2, [...chasFacultyData, ...coedFacultyData], JSON.parse(JSON.stringify(clonedCourseData)), mutationRate);
    ...
    const finalFitnessScores = population.map(chromosome => fitnessFunction(chromosome, [...chasFacultyData, ...coedFacultyData], clonedCourseData));
...
async function saveBestAssignment(bestChromosomeWithRooms, fitness, academicYear, selectedSemester) {
    ...
    const assignmentDocRef = doc(db, 'GenerateSchedule', 'CHAS_Schedule');
...
async function fetchRooms() {
    const roomData = [];
    const roomCollection = collection(db, 'Room_Data', 'Room_CHAS', 'rooms');
...
async function fetchAllDepartmentSchedules(academicYear, semester) {
    const departments = ['CBAA', 'COED', 'COE', 'CAS', 'CCS'];  // Add any other departments
    const schedules = [];
...
async function assignDayAndTimeWithExternalCheck(bestChromosomeWithRooms, academicYear, semester) {
    ...
    let noTimeSlotAlertShown = false;
    ...
    // Load existing schedules to check for conflicts
    ...
    // Assign day and time with the specified constraints
    bestChromosomeWithRooms.forEach(assignment => {
        const yearLevel = assignment.year_level;

        // Define available days based on year level
        let availableDays;
        if (yearLevel === "2nd Year" || yearLevel === "4th Year") {
        ...
        // Assign subjects across allowed days with a max of three subjects per day per section
...
export async function saveAndClearSchedule() {
    const sourceDocRef = doc(db, "GenerateSchedule", "CHAS_Schedule");
    ...
    try {
        ...
        if (scheduleDoc.exists()) {
            ...
            if (!academicYear || !selectedSemester) {
                console.error("Missing academicYear or selectedSemester in CHAS_Schedule document.");
                return;
            }
            ...
            console.log(`Schedule copied to finalSchedule > A.Y. ${academicYear} > ${selectedSemester} > CHAS > schedule.`);
            ...
            console.log("Fields cleared in GenerateSchedule > CHAS_Schedule.");
        } else {
...
export async function main() {
    try {
        ...
        console.log('Fetching CHAS faculty data...');
        const chasFacultyData = await fetchFaculty();
        console.log(`Fetched ${chasFacultyData.length} CHAS faculty members.`);
        ...
        let { bestChromosome, bestFitness } = geneticAlgorithm(chasFacultyData, coedFacultyData, courseData, sectionData, 50, 100, 0.05);
