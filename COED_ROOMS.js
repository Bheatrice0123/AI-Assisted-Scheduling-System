...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    document.getElementById('confirmEdit').addEventListener('click', async function() {
        ...
        try {
            // Reference to the Room_COED document in the Firestore
            const roomDocRef = doc(db, 'Room_Data', 'Room_COED', 'rooms', docId);
    ...
    async function doesRoomExist(roomNumber, campus, roomType, excludeDocId = null) {
        try {
            const roomDocRef = doc(db, 'Room_Data', 'Room_COED');
    ...
    async function saveRoomToFirestore(campus, roomType, roomNumber) {
        try {
            // Reference to the Room_COED document
            const roomDocRef = doc(db, 'Room_Data', 'Room_COED');
    
            // Reference to the sub-collection 'rooms' under Room_COED document
            const roomCollectionRef = collection(roomDocRef, 'rooms');
            ...
            if (!lastDoc.empty) {
                ...
                const lastId = parseInt(lastDocData.id.replace('roomCOED_', '')); // Extract the numeric part of the ID
                newId = lastId + 1; // Increment to get the new ID
            }
    
            // New document ID with leading zeros (e.g., sectionCOED_001)
            const newDocId = `roomCOED_${newId.toString().padStart(3, '0')}`;
